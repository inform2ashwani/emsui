...
const initialValue = {   
  ...
  function DepartmentName() {
    ...
    const columnDefs = [
      ...
      {headerName: 'Department Name',field :"name",width: 150, sortable: true}, 
      {headerName: 'isActive',field :"isActive",width: 150, sortable: true},      
   ...
   const getUsers = async () => {
    await axios.get(url+`/SubMaster?masterid=5`) 
    .then(resp =>{
     console.log(resp)
  ...
  const handleDelete = (id) => {   
    ...
    if (confirm) {
      ...
      axios.delete(url+`/SubMaster?masterid=5&SubMasterID=${id}`)
      .then(resp => getUsers())
  ...
  const handleFormSubmit = () => {
    ...
    if (formData.id) {
      ...
      confirm && fetch(url+`/SubMaster?masterid=5`, {       
      ...
      //adding new user   
      fetch(url+`/SubMaster?masterid=5`,{
        method: "POST", headers: {
          'content-type': "application/json"
        },
  ...
  const defaultColDef = {
   ...
   return (    
      ...
      <AddEditDepartment open={open} handleClose={handleClose}
