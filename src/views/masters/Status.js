...
const Status = () => {
  ...
  const classes =useStyles();
    ...
    const columns=[
      {headerName:"ID",field:'statusID',width: 70},
      {headerName: 'Status',field: "equipmentStatus",width: 100, sortable: true},     
  ...
  const datasource = {
    ...
    axios.get(url)
    .then(response => {
     console.log(response.data.statusLookup)
     params.successCallback(response.data.statusLookup, 499);             
    })
        .catch(err=>{
            console.log(err);
        })
