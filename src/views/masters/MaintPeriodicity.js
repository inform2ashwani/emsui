...
const MaintPeriodicity  = () => {
  ...
  const classes =useStyles();
    ...
    const columns=[
      {headerName:"ID",field:'maintPerodicityID',width: 70},
      
      {headerName: 'Periodicity',field :"periodicity",width: 150, sortable: true},       
      {headerName: 'CreatedDate',field:"CreatedDate",width: 110,cellRenderer: (data) => {
  ...
  const datasource = {
    ...
    axios.get(url)
    .then(response => {
     console.log(response.data.maintenancePeriodLookup)
     params.successCallback(response.data.maintenancePeriodLookup, 499);             
    })
        .catch(err=>{
            console.log(err);
        })
...
export default MaintPeriodicity ;